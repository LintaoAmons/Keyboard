import { KeyboardKeyNew, toKeyboardLayoutNew } from "./configParser";
...
describe('toKeyboardLayout', () => {
    test('should correctly parse a simple layout', () => {
        ...
        const layout = toKeyboardLayoutNew(layoutString);

        expect(layout.length).toBe(2);
        expect(layout[0][0]).toEqual(new KeyboardKeyNew('a', 2));
        expect(layout[1][1]).toEqual(new KeyboardKeyNew('e', 2, new Map([["f", true]])));
    });
